...
export default function Homepage() {
  ...
  const fetchAllPosts = async (page, nextValue, previousValue) => {
    ...
    if (searchValue === "cat") {
      ...
      response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/posts${search}&currentPage=${page}`
      );
      ...
      response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/posts?currentPage=${page}`
      );
    }
  ...
  return (
    <>
      ...
      <MiniCategories />
      <div className="home">
        {isLoading ? <LoadingSpinner /> : <Posts posts={posts} />}
        <Sidebar />
      </div>
      <PageNumbers
